<template>
  <div class="space-y-6">
    <div
      class="bg-gradient-to-r from-cyan-50/80 to-blue-50/80 border border-slate-100 rounded-2xl p-6 flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6"
    >
      <div
        class="w-20 h-20 rounded-2xl bg-gradient-to-tr from-cyan-500 to-blue-600 text-white flex items-center justify-center font-black text-3xl shadow-lg shadow-cyan-200 shrink-0"
      >
      ...
      <div class="text-center sm:text-left flex-1 min-w-0 space-y-1">
        <div class="flex items-center justify-center sm:justify-start space-x-2">
          <h2 class="text-xl font-bold text-slate-800 truncate">
            {{ userData.name || 'Đang tải thông tin...' }}
          </h2>
          <span
            class="px-2.5 py-0.5 text-[10px] font-extrabold uppercase rounded-full bg-emerald-100 text-emerald-700"
          >
            Đang hoạt động
          </span>
        </div>
    ...
    <div class="space-y-4">
      <h3
        class="text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100 pb-2"
      >
        Thông tin cá nhân
      </h3>
