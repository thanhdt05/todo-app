    ...
    <div class="w-full max-w-2xl bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
      ...
      <div class="p-6 sm:p-10">
        ...
        <ul class="space-y-2.5">
          <li v-for="task in filterTasks" :key="task.id" class="flex items-center justify-between px-4 py-3.5 bg-[#f4f6f7] rounded-xl">
            <div class="flex items-center space-x-3.5 flex-1 min-w-0 pr-3">
              <input type="checkbox" :checked="task.status === 'done'" @change="handleToggleTask(task)" class="w-5 h-5 accent-blue-600 rounded cursor-pointer" />
              <span :class="task.status === 'done' ? 'line-through text-slate-400' : 'text-sm sm:text-base text-slate-700'">{{ task.title }}</span>
            </div>
          </li>
        </ul>
