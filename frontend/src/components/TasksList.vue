<template>
  <div class="min-h-screen flex flex-col items-center justify-center p-4 sm:p-8 font-sans">
    ...
    <!-- Main Card Container -->
    <div class="w-full max-w-3xl bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
      ...
      <div class="p-6 sm:p-10">
        ...
        <button 
          type="button" 
          @click="openModal"
          class="px-5 py-2.5 bg-cyan-500 hover:bg-cyan-600 text-white font-bold rounded-xl shadow-md transition cursor-pointer flex items-center space-x-2 mb-10"
        >
    ...
    <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4 border border-slate-100">
        ...
        <h3 class="text-lg font-bold text-slate-800">Thêm công việc mới</h3>
        ...
        <!-- Form Nhập Liệu -->
        <form @submit.prevent="handleAddNewTask" class="space-y-4">
        <div>
...
const filterTasks = computed(() => {
    if (currentTab.value === 'TODO') {
        return tasks.value.filter(tasks => tasks.status === 'todo')
    } else if (currentTab.value === 'DOING') {
        return tasks.value.filter(tasks => tasks.status === 'doing')
    } else if (currentTab.value === 'DONE') {
        return tasks.value.filter(tasks => tasks.status === 'done')
    } else {
        return tasks.value
    }
})
        ...
        if (error.response?.data) {
            if (error.response.data.errors) {
                ...
                if (firstErrorKey) {
                    ...
                    errorMessage.value = error.response.data.message || 'Lấy danh sách công việc thất bại'
                }
            } else {
                errorMessage.value = error.response.data.message || 'Lấy danh sách công việc thất bại'
            }
        ...
        if (error.response?.data) {
            if (error.response.data.errors) {
                ...
                if (firstErrorKey) {
                    ...
                    errorMessage.value = error.response.data.message || 'Cập nhật công việc thất bại'
                }
            } else {
                errorMessage.value = error.response.data.message || 'Cập nhật công việc thất bại'
            }
        ...
        if (error.response?.data) {
            if (error.response.data.errors) {
                ...
                if (firstErrorKey) {
                    ...
                    errorMessage.value = error.response.data.message || 'Thêm mới công việc thất bại'
                }
            } else {
                errorMessage.value = error.response.data.message || 'Thêm mới công việc thất bại'
            }
        ...
        if (error.response?.data) {
            if (error.response.data.errors) {
                ...
                if (firstErrorKey) {
                    ...
                    errorMessage.value = error.response.data.message || 'Xóa công việc thất bại'
                }
            } else {
                errorMessage.value = error.response.data.message || 'Xóa công việc thất bại'
            }
